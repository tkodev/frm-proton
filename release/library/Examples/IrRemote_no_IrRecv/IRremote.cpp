...
IRsend::IRsend(int irPins) : irPin(irPin) {
  //
};
...
void IRsend::sendRC5(unsigned long data, int nbits){ // Note: first bit must be a one (start bit)
  enableIROut(36);
  data = data << (32 - nbits);
...
void IRsend::sendRC6(unsigned long data, int nbits){ // Caller needs to take care of flipping the toggle bit
  enableIROut(36);
  data = data << (32 - nbits);
  mark(RC6_HDR_MARK);
  space(RC6_HDR_SPACE);
 ...
 * The Dish send function needs to be repeated 4 times, and the Sharp function has the necessary repeat built in because of the need to invert the signal.
 *
