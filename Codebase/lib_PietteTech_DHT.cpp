...
String PietteTech_DHT::resultStr(int result){
  String status = "OK";
  if( result==DHTLIB_OK ){
  }else if( result==DHTLIB_ERROR_CHECKSUM ){
    status = "Error: DHT Checksum";
  }else if( result==DHTLIB_ERROR_ISR_TIMEOUT ){
    status = "Error: DHT ISR time out";
  }else if( result==DHTLIB_ERROR_RESPONSE_TIMEOUT ){
    status = "Error: DHT Response time out";
  }else if( result==DHTLIB_ERROR_DATA_TIMEOUT ){
    status = "Error: DHT Data time";
  }else if( result==DHTLIB_ERROR_ACQUIRING ){
    status = "Error: DHT Acquiring";
  }else if( result==DHTLIB_ERROR_DELTA ){
    status = "Error: DHT Delta time to small";
  }else if( result==DHTLIB_ERROR_NOTSTARTED ){
    status = "Error: DHT Not started";
  }else{
    status = "Error: DHT Unknown error code: "+String(status);
  }
  return status;
}
